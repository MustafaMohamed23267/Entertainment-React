...
export const Movies = ()=>
    {
        ...
        useEffect(()=>
            {
                // fetch("http://127.0.0.1:8000/api/movies").then((Response)=>Response.json()).then((json)=>setData(json));
            ...
            const handeltype = async (e)=>
                {
                    ...
                    const apitype = await axios.get(`http://127.0.0.1:8000/api/movies?selecttype=${enteredtype}`,{
                    headers:{
                            Accept:"application/json",
                            Authorization: `Bearer ${localStorage.getItem("token")}`
                        }
            ...
            const handelsearch = async (e)=>
                {
                    ...
                    setSearch(value);
                    const searchget = await axios.get(`http://127.0.0.1:8000/api/movies?searchname=${value}`,{
                    headers:{
                            Accept:"application/json",
                            Authorization: `Bearer ${localStorage.getItem("token")}`
                        }
       ...
       // const[rate , setRate] = useState("all");
        ...
        //         setRate(ratevalue);
        //         const getDataByRate = await axios.get(`http://127.0.0.1:8000/api/movies?rate=${ratevalue}`);
        ...
        //     }
            ...
            const handelRate = async ()=>
            {
                ...
                const getDataByRate2 = await axios.get(`http://127.0.0.1:8000/api/movies?rate1=${rate1}&rate2= ${rate2}`,{
                    headers:{
                            Accept:"application/json",
                            Authorization: `Bearer ${localStorage.getItem("token")}`
                        }
...
{data.map(data=>
        ...
        <div key={data.id} className="flex flex-col  duration-700 outline-1 -outline-offset-1 outline-gray-300 dark:outline-white/10 dark:bg-white/5  h-[700px] overflow-hidden  rounded-xl ">
             <Link to={`/movies/${data.id}`}  class=" space-y-5  ">
                 <div>
                    <img src={`http://127.0.0.1:8000/storage/${data.image}`} class="duration-500  hover:scale-[1.05] w-[100%]  z-50 overflow-hidden" />
                </div>
