...
export const AnimeEdit = ()=>
    {
        ...
        useEffect(()=>{
            fetch(`http://127.0.0.1:8000/api/anime/${id}`)
            .then((res)=>res.json())
        ...
        const HandelUpdate = async (e)=>{
            ...
            const updateres = await fetch(`http://127.0.0.1:8000/api/anime/${id}`,{
                method:"POST",
                body:DataForm,
                headers:{
        ...
        return(
            ...
            <br/>
             <head>
                                    <title>{`Edit ${showAnime.name} `}</title>
                                </head>
            <form onSubmit={HandelUpdate} className="space-y-2 flex  flex-col items-center pt-25 text-xl p-2 w-full h-[100vh] ">
                <p class="text-center text-4xl duration-700 font-bold">Updating {showAnime.name}</p>
            ...
            <div class="flex space-x-4 ">
                ...
                <div className="px-20 py-6 w-full flex justify-center">
                    <button type="submit" className="  bg-yellow-500 px-40 py-1 rounded-xl cursor-pointer hover:bg-green-700 duration-500">Update</button>
