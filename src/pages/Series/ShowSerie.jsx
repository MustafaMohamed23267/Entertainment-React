...
const fetchSeries = async()=>{
                    const res = await fetch(`http://127.0.0.1:8000/api/series/${id}`,{
                        method:"GET",
                        headers:{
...
const HandelDelete = async (e)=>{
    e.preventDefault();
    await fetch(`http://127.0.0.1:8000/api/series/${id}`,{
        method:"DELETE",
        headers:{
            Accept:"application/json",
            Authorization: `Bearer ${localStorage.getItem("token")}`
...
const fullopacity = document.getElementById("deletemodal").style.display="none" ;
    ...
    <div class="grid grid-cols-2 md:grid-cols-2">
       ...
       <img src={`http://127.0.0.1:8000/storage/${ShowSerie.image}`} className="w-full " />
